...
def test_validate_password(cached_password_policies):
    response = britive.secrets_manager.password_policies.validate(cached_password_policies['id'], 'testpassword')
...
def test_static_secret_template_update(cached_static_secret_template):
    ...
    assert (
        britive.secrets_manager.static_secret_templates.get(cached_static_secret_template['id'])['description']
        == 'test desc'
    )
...
def test_secret_get(cached_vault):
    secret = britive.secrets_manager.secrets.get(vault_id=cached_vault['id'], path='/', secret_type='secret')[0]
    assert isinstance(secret, dict)


def test_secret_update(cached_secret, cached_vault):
    update = britive.secrets_manager.secrets.update(
        cached_vault['id'], cached_secret['path'], {'Note': 'updated test note'}
    )
...
def test_secret_rename(cached_secret, cached_vault):
    ...
    rename = britive.secrets_manager.secrets.rename(cached_vault['id'], cached_secret['path'], 'testSecretRename')
    ...
    rollback = britive.secrets_manager.secrets.rename(cached_vault['id'], new_path, name)
