...
def test_get(cached_access_broker_profile_policy, cached_access_broker_profile):
    policy = britive.access_broker.profiles.policies.get(policy_id=cached_access_broker_profile_policy['id'], profile_id=cached_access_broker_profile['profileId'])
    assert isinstance(policy, dict)

def test_update(cached_access_broker_profile_policy, cached_access_broker_profile):
    britive.access_broker.profiles.policies.update(
        ...
        members={'users' : [{"id": "t3zd4g9teboomymyx4q4"}]}
    )
    policy = britive.access_broker.profiles.policies.get(policy_id=cached_access_broker_profile_policy['id'], profile_id=cached_access_broker_profile['profileId'])
    assert isinstance(policy, dict)
    assert policy['description'] == 'test2'