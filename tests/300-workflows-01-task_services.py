...
def test_enable(cached_task_service):
    response = britive.workflows.task_services.enable(task_service_id=cached_task_service['taskServiceId'])
    assert isinstance(response, dict)
    assert response['enabled']


def test_disable(cached_task_service):
    response = britive.workflows.task_services.disable(task_service_id=cached_task_service['taskServiceId'])
    assert isinstance(response, dict)
