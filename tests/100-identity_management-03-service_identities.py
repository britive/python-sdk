...
def test_list(cached_service_identity):
    response = britive.identity_management.service_identities.list()
...
def test_get(cached_service_identity):
    user = britive.identity_management.service_identities.get(cached_service_identity['userId'])
    assert isinstance(user, dict)
    assert set(service_identity_keys).issubset(user.keys())


def test_get_by_name_co(cached_service_identity):
    users = britive.identity_management.service_identities.get_by_name(cached_service_identity['name'])
...
def test_search(cached_service_identity):
    users = britive.identity_management.service_identities.search(cached_service_identity['name'].split('@')[0])
    assert isinstance(users, list)
    assert len(users) > 0
    assert isinstance(users[0], dict)
    assert set(service_identity_keys).issubset(users[0].keys())
...
def test_get_by_status():
    users = britive.identity_management.service_identities.get_by_status('active')
    assert isinstance(users, list)
    assert len(users) > 0
    assert isinstance(users[0], dict)
    assert set(service_identity_keys).issubset(users[0].keys())
...
def test_update(cached_service_identity):
    user = britive.identity_management.service_identities.update(cached_service_identity['userId'], description='test2')
    assert isinstance(user, dict)
    assert user['description'] == 'test2'


def test_disable_single(cached_service_identity):
    user = britive.identity_management.service_identities.disable(service_identity_id=cached_service_identity['userId'])
    assert isinstance(user, dict)
    assert user['status'] == 'inactive'


def test_enable_single(cached_service_identity):
    user = britive.identity_management.service_identities.enable(service_identity_id=cached_service_identity['userId'])
    assert isinstance(user, dict)
    assert user['status'] == 'active'


def test_disable_list(cached_service_identity):
    user = britive.identity_management.service_identities.disable(
        service_identity_id=cached_service_identity['userId'], service_identity_ids=[cached_service_identity['userId']]
    )
...
def test_enable_list(cached_service_identity):
    user = britive.identity_management.service_identities.enable(
        service_identity_id=cached_service_identity['userId'], service_identity_ids=[cached_service_identity['userId']]
    )
    assert isinstance(user, list)
...
def test_remove_custom_identity_attributes(cached_service_identity, cached_identity_attribute):
    ...
    response = britive.identity_management.service_identities.custom_attributes.remove(
        principal_id=cached_service_identity['userId'], custom_attributes={cached_identity_attribute['name']: [value]}
    )
    assert response is None
