...
def test_query_json():
    events = britive.audit_logs.query(from_time=(datetime.now() - timedelta(1)), to_time=datetime.now())
...
def test_query_csv():
    csv = britive.audit_logs.query(from_time=datetime.now() - timedelta(1), to_time=datetime.now(), csv=True)
