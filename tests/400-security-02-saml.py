...
def test_settings():
    settings = britive.security.saml.settings()
    ...
    settings = britive.security.saml.settings(as_list=True)
    assert isinstance(settings, list)
    assert len(settings) == 1


def test_metadata():
    metadata = britive.security.saml.metadata()
    assert isinstance(metadata, str)
    assert 'ds:X509Certificate' in metadata


def test_download():
    certificate = britive.security.saml.certificate()
