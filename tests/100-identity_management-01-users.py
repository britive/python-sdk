...
def test_list(cached_user):
    response = britive.identity_management.users.list()
...
def test_get(cached_user):
    user = britive.identity_management.users.get(cached_user['userId'])
...
def test_get_by_name_co(cached_user):
    users = britive.identity_management.users.get_by_name(cached_user['lastName'])
...
def test_search(cached_user):
    users = britive.identity_management.users.search(cached_user['email'].split('@')[0])
    assert isinstance(users, list)
    assert len(users) > 0
    assert isinstance(users[0], dict)
    assert user_keys.issubset(users[0])
...
def test_get_by_status():
    users = britive.identity_management.users.get_by_status('active')
    assert isinstance(users, list)
    assert len(users) > 0
    assert isinstance(users[0], dict)
    assert user_keys.issubset(users[0])
...
def test_update(cached_user):
    user = britive.identity_management.users.update(cached_user['userId'], mobile='1234567890')
    assert isinstance(user, dict)
    assert user['mobile'] == '1234567890'


def test_disable_single(cached_user):
    user = britive.identity_management.users.disable(user_id=cached_user['userId'])
    assert isinstance(user, dict)
    assert user['status'] == 'inactive'


def test_enable_single(cached_user):
    user = britive.identity_management.users.enable(user_id=cached_user['userId'])
    assert isinstance(user, dict)
    assert user['status'] == 'active'


def test_disable_list(cached_user):
    user = britive.identity_management.users.disable(user_id=cached_user['userId'], user_ids=[cached_user['userId']])
    assert isinstance(user, list)
    assert user[0]['status'] == 'inactive'


def test_enable_list(cached_user):
    user = britive.identity_management.users.enable(user_id=cached_user['userId'], user_ids=[cached_user['userId']])
    assert isinstance(user, list)
...
def test_reset_mfa(cached_user):
    with pytest.raises(UserDoesNotHaveMFAEnabled):
        britive.identity_management.users.reset_mfa(cached_user['userId'])


def test_set_custom_identity_attributes(cached_user, cached_identity_attribute):
    response = britive.identity_management.service_identities.custom_attributes.add(
        principal_id=cached_user['userId'],
        custom_attributes={cached_identity_attribute['id']: [f'test-attr-value-{random.randint(0, 1000000)}']},
    )
    assert response is None


def test_get_custom_identity_attributes_list(cached_user, cached_identity_attribute):
    response = britive.identity_management.service_identities.custom_attributes.get(
        principal_id=cached_user['userId'], as_dict=False
    )
    assert isinstance(response, list)
...
def test_get_custom_identity_attributes_dict(cached_user, cached_identity_attribute):
    response = britive.identity_management.service_identities.custom_attributes.get(
        principal_id=cached_user['userId'], as_dict=True
    )
...
def test_remove_custom_identity_attributes(cached_user, cached_identity_attribute):
    value = britive.identity_management.service_identities.custom_attributes.get(
        principal_id=cached_user['userId'], as_dict=True
    )[cached_identity_attribute['id']]
    response = britive.identity_management.service_identities.custom_attributes.remove(
        principal_id=cached_user['userId'], custom_attributes={cached_identity_attribute['name']: [value]}
    )
    assert response is None

    attributes = britive.identity_management.service_identities.custom_attributes.get(
        principal_id=cached_user['userId'], as_dict=False
    )
...
def test_minimized_user_details(cached_user):
    details = britive.identity_management.users.minimized_user_details(user_id=cached_user['userId'])
    assert isinstance(details, list)
    assert len(details) == 1
    details = britive.identity_management.users.minimized_user_details(user_ids=[cached_user['userId']])
    assert isinstance(details, list)
    assert len(details) == 1
